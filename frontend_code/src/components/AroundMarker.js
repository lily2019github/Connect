...
class AroundMarker extends React.Component {
    ...
    render() {
        const { user, message, src, location, type } = this.props.post;
            ...
            >
                {this.state.isOpen ? (
                    <InfoWindow>
                        <div>
                            {isImagePost
                                ? <img src={src} alt={message} className="around-marker-image"/>
                                : <video src={src} controls className="around-marker-video"/>}
                            <p>{`${user}: ${message}`}</p>
                        </div>
                    </InfoWindow>
                ) : null}
