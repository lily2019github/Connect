...
func handlerCluster(w http.ResponseWriter, r *http.Request) {
    ...
    w.Header().Set("Content-Type", "applicsavePostation/json")
...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    medialink, err := saveToGCS(file, p.Id)
    if err != nil {
    ...
    if p.Type == "image" {
        uri := fmt.Sprintf("gs://%s/%s", BUCKET_NAME, p.Id)
    ...
    err = saveToES(p, POST_INDEX, p.Id)
    if err != nil {
        http.Error(w, "Failed to save post to GCS or Elasticsearch", http.StatusInternalServerError)
        fmt.Printf("Failed to save post to GCS or Elasticsearch %v\n", err)
        return
...
func searchHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if user != "" {
        ...
        fmt.Println("search by keyword or all")
